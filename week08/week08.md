...
## What is ETL?
- Extract, transform and load (ETL) is the process of combining data from multiple sources into a data warehouse. ETL uses a set of business rule to clean and organise raw data to prepare it for storage, analytics and machine learning. Specific business intelligence need can be addressed through analytics (such as predicting the outcome of business decision, generating report and dashboard, reducing operational inefficiency etc)

- 🤔 Why is ETL important?

  - Organisations today have both structured and unstructured data from various sources; by applying ETL, individual raw dataset can be prepared in a format and structure that is more consumable for analytics purpose, resulting in more meaningful insight


## How does ETL work?
![ETL](etl-architecture.svg)<!-- .element: height="300px" -->

- ETL works by moving data from the source system to the destination system at periodic interval. The ETL process works in three steps:
  1. Extract the relevant data from the source
  2. Transform the data so that it is better suited for analytics
  3. Load the data into the target data repository


## Data extraction
- In data extraction, ETL tool extracts or copies raw data from multiple sources and store them in a staging area. A staging area is an intermediate data store for extracted data. Theys are often transient, meaning the data is erased after ETL is complete. However, the staging area might also retain a data archive for troubleshooting etc
...
- In data transformation, ETL tool transforms and consolidates the raw data in the staging area to prepare it for the target data repository
...
- **Derivation**: applies business rule to your data to calculate new value from existing value. For example, you can convert revenue to profit by subtracting total cost
...
- **Encryption**: protects sensitive data to comply with legal requirement by adding encryption before the data streams to the target data repository


## Data loading
- In data loading, ETL tool moves the transformed data from the staging area into the target data repository. In most cases, this process is automated, well defined, continual and batch driven. There are two approaches:

  - Full load: the entire data from the source is transformed and moved to the data repository. The full load usually takes place the *first time* you load data from a source system into the data warehouse
  - Incremental load: the ETL tool loads the *delta* between target and source systems at regular interval. It stores the last extract date so that only rows added after this date are loaded


## What is ELT?
- Extract, load and transform (ELT) is an extension of ETL that reverses the order of operation. You can load data directly into the target data repository before processing it. The intermediate staging area is not required because the target data warehouse or data mart has data mapping capabilities within it. ELT has become more popular with the adoption of cloud infrastructure which gives target database the processing power they need for transformation
...
- The ETL process requires more definition at the beginning. Analytics needs to be involved from the start to define target data type, structure, and relationship. Data scientist mainly uses ETL to load legacy database into the warehouse, and ELT has become the norm today
...
- While SQLite is not considered an optimised [OLAP](https://en.wikipedia.org/wiki/Online_analytical_processing) solution like [DuckDB](https://duckdb.org/), it is an excellent tool for ETL / ELT prototyping with SQL. Also, it could act as the local staging intermediate data store before the cloud, supporting both offine-first or embedded ETL workflows
...
- If a SQLite database is a data repository for ELT, then the objective of data extraction is to identify and acquire the dataset through specific website, API etc in CSV (or TSV) file format
...
Note: The major difference between using list mode with comma as separator and the csv mode is that the csv mode applies text qualifer (i.e. a special character, usually the double quote) to enclose all text-based values, and that feature could be very important particularly if the values actually contain the comma characters among them; separator is also known as delimiter
...
- The objective data transformation in SQLite is to transform and consolidate the loaded data using SQL function and technique; it should be guided by the star schema design if the outcome is to create a data warehouse / mart

- This stage would spend most of the time and effort comparing to data extraction and data loading
...
- By now you have learnt:
	...
	- what ETL and ELT are, and how they are different
